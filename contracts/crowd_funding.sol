...
contract crowd_funding{
    ...
    mapping (address => uint256) public creatorsAddressToID;
    mapping (uint256 => Creator) idToCreators;
    uint56 noOfCreators = 0;


    //creator functions
    function RegisterCreator(string calldata name,string calldata category,uint256 milestoneGoal) public {
    ...
    // UploadProjects
    function UploadProject(string calldata name)public{ //need to specify project fund ??
    ...
    function InitiateVoting(uint256 amount)public {
        ...
        require(amount < redeemableAmount,"You donot have enough redeemable amount");

        VotingVenture storage previousVenture =  creator.idToVotingVenture[creator.noOfVotingVentures];
        require(previousVenture.isVentureActive == false,"A voting venture is already on going! Cannot create a new one");

        VotingVenture storage venture =  creator.idToVotingVenture[creator.noOfVotingVentures++];
    ...
    mapping (address => uint256) public contributorsAddressToID;
    mapping (uint256 => Contributor) public idToContributor;
    ...
    function Contribute(address creatorAddress) public payable {
        ...
        //     uint256 amountInCurrent = currentMilestone.goal - currentMilestone.fundRaised;
        ...
        //     currentMilestone.contributorIdToAmount[contributorId] += amountInCurrent;
        ...
        //         idToContributor[id].claimableAmount += currentMilestone.contributorIdToAmount[id]*5/100;
        ...
        //     currentMilestone.contributorsList.push(contributorId);
        //     currentMilestone.contributorIdToAmount[contributorId] = amountInNext;
        //     currentMilestone.noOfContributors++;
        // }

        //Update Membership Strength
        // ... here ...
        contributor.creatorIdToMembershipStrength[creatorId] += amount * creator.totalFundRaised;
    ...
    //     uint256 noOfVotingVentures;
        ...
        uint256 contributorId = contributorsAddressToID[msg.sender];
        uint256 creatorId = creatorsAddressToID[creatorAddress];
        VotingVenture storage currentVoting = idToCreators[creatorId].idToVotingVenture[idToCreators[creatorId].noOfVotingVentures-1];

        require(currentVoting.isVentureActive, "Voting Venture is not active");        

        // isMember
        require(idToContributor[contributorId].creatorIdToMembershipStrength[creatorId] > 0);       
        ...
        if(isUpvote){
            ...
            currentVoting.upScore += idToContributor[contributorId].creatorIdToMembershipStrength[creatorId];

        }else{

            currentVoting.downScore += idToContributor[contributorId].creatorIdToMembershipStrength[creatorId];

        }

        // Update membership strength
        idToContributor[contributorId].creatorIdToMembershipStrength[creatorId] += 1 * idToCreators[creatorId].totalFundRaised;
    ...
    function getMyVotingVentureResult()public returns (bool) {
        ...
        Creator storage creator = idToCreators[creatorId];
        VotingVenture storage currentVoting = creator.idToVotingVenture[idToCreators[creatorId].noOfVotingVentures-1];

        require(currentVoting.isVentureActive,"No active venture");
        
        require(currentVoting.noOfVoters * 2 > creator.noOfContributors,"Not enough voters");
    ...
    function getMemebershipStrength(uint256 creatorId)public view  returns (uint256){
        uint256 contributorId = contributorsAddressToID[msg.sender];
        return idToContributor[contributorId].creatorIdToMembershipStrength[creatorId];
    }

    function currentMilesoneNumber(uint256 creatorId)public view returns(uint256){
